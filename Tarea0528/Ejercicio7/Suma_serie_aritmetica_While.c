...
int main(){
    printf("Programa para calcular la suma de los primeros n terminos de una serie aritmetica con While\n"); //Indico que tipo de programa estoy realizando
    int numeros,ptermino,snumeros,diferencia,aux=0; //Defino variables
    printf("Escribe cuantos terminos va a tener la serie aritmerica\n"); //Indico que el usuario ingrese un numero
    ...
    while(aux<numeros){ //Establezco mi condicion para buscar que el bucle termine cuando el valor de aux sea mayor a el valor de numeros
        if (aux==(numeros-1)){ //Estructura Condicional Multiple para indicar cuando debe de mostar el +
            printf("%+d=", ptermino);
        } else{
            if(ptermino<0 || aux==0){
                printf("%d", ptermino);
